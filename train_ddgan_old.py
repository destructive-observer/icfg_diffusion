...
def train(rank, gpu, args):
    ...
    if args.dataset == 'cifar10':
        ...
        dataset = ImageFolder('/raid/datasets/celeba_hq/celeba-256/',transform=train_transform)
    ...
    for epoch in range(0, args.num_epoch+1):
        ...
        for iteration, (x, y) in enumerate(data_loader):
            ...
            real_data[2,:,:,:]=real_data[0,:,:,:]
            real_data[3,:,:,:]=real_data[0,:,:,:]
            #sample t
            t = torch.randint(0, args.num_timesteps, (4,), device=device)
            t[0]=0
            t[1]=1
            t[2]=2
            t[3]=3
            x_t, x_tp1 = q_sample_pairs(coeff, real_data, t)
            torchvision.utils.save_image(real_data, './sample_discrete_epoch__realreal_origin_{}.png')
            torchvision.utils.save_image(x_t, './sample_discrete_epoch__realreal_{}.png')
            torchvision.utils.save_image(x_tp1, './sample_discrete_epoch_realreal+1_{}.png')
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x, y) in enumerate(data_loader):
            ...
            x_t, x_tp1 = q_sample_pairs(coeff, real_data, t)
            torchvision.utils.save_image(x_t, os.path.join(exp_path, './sample_discrete_epoch_real_{}.png'.format(epoch)), normalize=True)
            torchvision.utils.save_image(x_tp1, os.path.join(exp_path, './sample_discrete_epoch_real+1_{}.png'.format(epoch)), normalize=True)
            x_t.requires_grad = True
            ...
            if rank == 0:
                ...
                # for i in range(2):
                #     torchvision.utils.save_image(fake_list[i], os.path.join(exp_path, 'sample_discrete_epoch_fake_{}.png'.format(epoch)), normalize=True)
                # t_s = (0)
                # x_t, x_tp1 = q_sample_pairs(coeff, real_data, t_s)
                # torchvision.utils.save_image(x_t, os.path.join(exp_path, 'sample_discrete_epoch_real_{}.png'.format(epoch)), normalize=True)
                # torchvision.utils.save_image(x_tp1, os.path.join(exp_path, 'sample_discrete_epoch_real+1_{}.png'.format(epoch)), normalize=True)
