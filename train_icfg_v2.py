...
def sample_from_model_icfg(coefficients, n_time, x_init,ddg,opt):
   ...
   # n_time=4 3 2 1   input
   # t_value 4:4/3:4,3/2:4,3,2/1:4,3,2,1/ 
    x = x_init
    list_time = opt.num_timesteps - n_time +1
   #  print(list_time)
   #  with torch.no_grad():
    for i in range(list_time):
      print(i)
      t_value =    opt.num_timesteps-i
      # print(t_value)
      t = torch.full((x.size(0),), t_value-1, dtype=torch.int64).to(x.device)
      x_0 = ddg.generate(opt.num_gen,real_data=x,list_or_not=False,t_time=t_value)[0]
      x_0 = x_0.cuda()
      x_new = sample_posterior(coefficients, x_0, x, t)
      x = x_new.detach()
...
#-----------------------------------------------------------------
def cfggan(opt, d_config, g_config, z_gen, loader,device,fromfile=None,saved=None,
...
#    check_opt_(opt)
   ...
   begin = 0
   ddg = DDG(opt, d_config, g_config, z_gen, optim_config,device,fromfile)
#    timeLog('saved ' + str(saved) + '.')
   ...
   if saved:
      ...
      ddg.initialize_G(g_loss, opt.cfg_N,loader,iterator,opt.num_timesteps)
   multi = 0
   epo = 1000
   #---  xICFG
   for t in reversed(range(begin+1,opt.num_timesteps+1)):     
     for stage in range(begin+multi,epo+multi):
      timeLog('xICFG stage %d -----------------' % (stage+1))
      iterator,diff,d_loss_v,d_loss_gp,d_fake,d_real = ddg.icfg(loader, iterator, d_loss, opt.cfg_U,t)
      ddg.epoch = stage
      ...
      if is_time_to_generate(opt, stage):
         generate(opt, ddg, stage,time_t=t)        
         
      ddg.approximate(g_loss, opt.cfg_N,loader,iterator,t)  
     multi +=epo       
...
#-----------------------------------------------------------------
class DDG:
   def __init__(self, opt, d_config, g_config, z_gen, optim_config,device, from_file=None):
      assert opt.cfg_T > 0
    ...
    #   self.logstr = opt.logstr
      ...
      self.epoch = 0
      self.device = device
      self.coeff = Diffusion_Coefficients(opt, self.device)
      self.pos_coeff = Posterior_Coefficients(opt, self.device)
   ...
   def get_real_list(self, t):
    #   self.check_t(t, 'get_d_params')
      return self.real_list[t]
   def store_real_list(self, t,real_data):
    #   self.check_t(t, 'get_d_params')
      # print(t,real_data.shape)
      # self.real_list[t]=real_data
      self.real_list.append(real_data)

    #   return self.real_list[t]  
   #----------------------------------------------------------      
   def generate(self, num_gen, t=-1, do_return_z=False, batch_size=-1,real_data=None,list_or_not=None,t_time=None):
      assert num_gen > 0
      if t < 0:
         t = self.num_D()
      if batch_size <= 0:
      ...
      while num_gened < num_gen:
         ...
         with torch.no_grad():       
            ...
            # print(fake.shape)
            # print(t)
            # fake = sample_posterior(self.pos_coeff, x_0_predict, x_tp1, t_emb)
        ...
        #        vizG_n.images(fake_g,opts=dict(title='0 - 1 fake images vizG_n+stage{}+numD full'.format(self.epoch), caption='vizG_n D.')) 
         ...
         t_emb_s [num_gened:num_gened+num] = t_emb.to(torch.device('cpu'))
   ...
   #-----------------------------------------------------------------
   def icfg(self, loader, iter, d_loss, cfg_U,t_time):   
    ...
    #   timeLog('DDG::icfg ... ICFG with settings=%s' % self.logstr)
      ...
      for t in range(self.num_D()):
         ...
         for upd in range(cfg_U):
            ...
            fake,t_emb,x_t,x_tp1 = self.generate(num, t=t,real_data=sample[0],list_or_not=False,t_time=t_time)
            ...
            # x_t, x_tp1 = q_sample_pairs(self.coeff, sample[0], t_emb)
            # print('x_t{}'.format(x_t))

            # to_pm1(x_tp1)
            # print('x_tp1{}'.format(x_tp1))
            # print('fake{}'.format(fake))
            d_out_real = self.d_net(x_t,self.d_params, t_emb, x_tp1.detach()).view(-1)
            d_out_fake = self.d_net(fake,self.d_params, t_emb, x_tp1.detach()).view(-1)
   ...
   #-----------------------------------------------------------------
   def initialize_G(self, g_loss, cfg_N,loader,iterator,t_time): 
      ...
      ## sample batch_size = 1
      t_emb = torch.randint(t_time-1, t_time, (1,),device=self.device)
      # print(t_emb)
      # print(sample[0][0].shape)
      # print(t_emb.shape)
      # print(self.coeff.device)
      # print(sample[0][0].device)
      # print(t_emb.device)
      x_t, x_tp1 = q_sample_pairs(self.coeff, real_data[0].unsqueeze(0), t_emb)
      x_0_predict = self.g_net(x_tp1.detach(),self.g_params, t_emb, z)
      ...
      # params = { 'proj.w':  variance_scaling_init_(torch.Tensor(z_dim, img_dim),scale=0.)}
      params['proj.w'].requires_grad = True
   ...
   #-----------------------------------------------------------------
   def approximate(self, g_loss, cfg_N,loader,iterator,t_time): 
      ...
      while num_gened < cfg_N:
         ...
         num_gened += num   
      target_fakes,zs,t_emb_s,x_t,x_tp_1= self.generate(cfg_N, do_return_z=True, batch_size=batch_size,real_data=sample_list,list_or_not=True,t_time=t_time)
      # print(target_fakes)
   ...
   def _approximate(self, loader, g_loss): 
      ...
      for epoch in range(self.optim_config.cfg_x_epo):
         ...
         for sample in loader:
            ...
            # print(t_emb.dtype)
            # t_emb = torch.randint(0, self.time_step, (target_fake.size(0),),device=self.device)
            # t_emb = t_emb.cuda()
            # if i == 0:
            ...
            # x_t, x_tp1 = q_sample_pairs(self.coeff, target_fake, t_emb)
            # x_t, x_tp1 = q_sample_pairs(self.coeff, target_real, t_emb)
            ...
            x_0_predict = self.g_net(x_tp1.detach(),g_params, t_emb, z)
            # x_pos_sample = sample_posterior(self.pos_coeff, x_0_predict, x_tp1, t_emb)

            # fake = self.g_net(z, g_params, is_train)
            ## ########## point 1: use targex_pos_sample_fake or x_0_predict
            loss = g_loss(x_0_predict, target_fake)
...
#-----------------------------------------------------------------
def generate(opt, ddg, stage='',l=1,time_t=0):
